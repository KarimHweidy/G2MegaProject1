...
void LED_HAL_Init(uint8_t copy_u8Port,uint8_t copy_u8Pin, uint8_t copy_u8Mode, uint8_t copy_u8ConnectionType )
{
	GPIO_u8PinInit(copy_u8Port, copy_u8Pin, copy_u8Mode);
   
	switch(copy_u8ConnectionType)
	{
	case LED_SourceConnnec	: GPIO_u8setpinValue(copy_u8Port, copy_u8Pin,PIN_LOW); break; /* turn off led*/
	case LED_SinkConnec  	: GPIO_u8setpinValue(copy_u8Port, copy_u8Pin,PIN_HIGH); break;  /* turn off led*/
	default: break;
	}
