...
uint8_t SPI_u8Init( SPI_Config_t* Copy_SPI)
{
	...
	ArrSPI[Copy_SPI->SPI_Num]->CR1 |= (Copy_SPI->ClockPolpha << CPHA_BIT ) | (Copy_SPI->DataSize << DFF_BIT ) | ( Copy_SPI->DataOrder <<LSB_BIT)  | ( Copy_SPI->Mode <<MSTR_BIT ); 
	...
	else 
	{
		...
		ArrSPI[Copy_SPI->SPI_Num]->CR1 |= ( Copy_SPI->NSS_mode << SSM_BIT);
